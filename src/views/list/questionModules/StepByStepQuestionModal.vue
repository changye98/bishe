        ...
        <div v-show="currentStep === 1">
          ...
          >
            <a-select v-decorator="['type', {rules: [{required: true}]}]" placeholder="请选择题型" style="width: 100%">
              <a-select-option v-for="typeObj in types" :value="typeObj.questionTypeId" :key="typeObj.questionTypeId">
                {{ typeObj.questionTypeDescription }}
              </a-select-option>
            </a-select>
          </a-form-item>
          ...
          >
            <a-select v-decorator="['category', { rules: [{required: true}]}]" placeholder="请选择分类" style="width: 100%">
              <a-select-option v-for="category in categories" :value="category.questionCategoryId" :key="category.questionCategoryId">
          ...
          >
            <a-select v-decorator="['level', { rules: [{required: true}]}]" placeholder="请选择难度" style="width: 100%">
              <a-select-option v-for="level in levels" :value="level.questionLevelId" :key="level.questionLevelId">
                {{ level.questionLevelDescription }}
              </a-select-option>
            </a-select>
          </a-form-item>
            <a-form-item
            ...
            >
                <a-input
                        v-decorator="['score', { rules: [{required: true}]}]"
                        placeholder="输入题目分数"
...
export default {
  ...
  methods: {
    create () {
      ...
      getAction("/question/selection").then(res => {
        if (res.data.success) {
          this.categories = res.data.result.categories
          this.levels = res.data.result.levels
    ...
    handleNext (step) {
      ...
      validateFields((errors, values) => { // values就是表单中校验的值，后面提交到后端接口时主要就是用这个values
        ...
        if (!errors) {
          ...
          values['creator']=localStorage.getItem("userId")
          console.log('values:', values)
          // 把data中的question属性提交到后端，待写完后端接口.把前端的创建的题型提交到后端
          postAction("/question/create",values).then(res => {
            // 成功就跳转到结果页面
