  ...
  <a-layout>
    <a-layout-header class="header" style="color: #fff">
      ...
      <span style="float: right;">
        <span style="margin-right: 60px; font-size: 20px" v-if="examDetail.exam">考试限时：{{ examDetail.exam.examTimeLimit }}分钟 |  剩余时间:{{restTime}}分钟</span>
    ...
    <a-layout>
      <a-layout-sider width="190" :style="{background: '#444',overflow: 'auto', height: '100vh',margin:'22px -150px 0px 20px', left: 0 }">
        <a-menu
          mode="inline"
...
export default {
  ...
  created () {
    ...
    // 从后端获取考试的详细信息，渲染到考试详情里
      getAction("/exam/detail/"+this.$route.params.id)
      .then(res => {
        if (res.data.success === true) {
          // 赋值考试对象
          that.examDetail = res.data.result
          that.restTime = that.examDetail.exam.examTimeLimit
        } else {
  ...
  methods: {
    getQuestionDetail (questionId) {
      ...
      this.radioValue = ''
      this.checkValues = []
      getAction("/exam/question/detail/"+questionId).then(res => {
        console.log(res)
        if (res.data.success === true) {
          // 赋值当前考试对象
    ...
    finishExam () {
      const  that = this;
        this.$confirm({
          title: '提示',
          content: '确定提交试卷吗?检查了吗?',
          okText:"确定",
          cancelText:"取消",
          onOk() {
            postAction("/exam/finish/"+that.$route.params.id+"/"+localStorage.getItem("userId"),that.mapToJson(that.answersMap)).then(res => {
              if (res.data.success === true) {
                // 考试交卷，后端判分完成，然后跳转到我的考试界面
                that.$notification.success({
                  message: '考卷提交成功！'
                })
                that.$router.push('/home/myGrade')
              } else {
